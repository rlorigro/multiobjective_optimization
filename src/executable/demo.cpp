...
void generate_data(
    ...
    for (int i=0; i<n; i++){
        ...
        // Construct the distribution with sigma param
        normal_distribution<double> distribution_a(0,sigma);
...
void plot(
    ...
    int64_t size = (v_max - v_min)*int64_t(pow(10,precision));
...
void parse_solution(
    ...
    plot(1, v_min, v_max, coords, colors, "input_" + name + ".ppm");

    unordered_map<int64_t,int64_t> id_to_parent;

    ofstream nodes_file("nodes_" + name + ".csv");
    ofstream edges_file("edges_" + name + ".csv");
    ...
    if (response.status() == CpSolverStatus::OPTIMAL || response.status() == CpSolverStatus::FEASIBLE) {
        ...
        for (const auto& [id,var]: vars.is_parent){
            ...
            if (is_parent){
                auto p = int64_t(id_to_parent.size());
                id_to_parent[id] = p;
            }
        }
    ...
    plot(1, v_min, v_max, coords, colors, "result_" + name + ".ppm");
}


void jointly_optimize(
        CpModelBuilder& model,
        Variables& vars,
        CpSolverResponse& response_n,
        CpSolverResponse& response_d,
        CpSolverResponse& response_n_d){
    cerr << "Finding first pareto bound..." << '\n';

    // First find one extreme of the pareto set
    model.Minimize(vars.cost_d);

    response_d = Solve(model.Build());
...
//    const CpSolverResponse response_n_d = SolveWithParameters(model.Build(), parameters);
    ...
    response_n_d = Solve(model.Build());
}
...
int main(int argc, char** argv){
    ...
    if (argc == 1){
        ...
        throw runtime_error("ERROR: too many arguments. Only an int64 seed is accepted.");
    }
    ...
    generate_data(v_min, v_max, n, n_samples, seed, sigma, coords, offset_coords, labels);

    cerr << "Constructing model..." << '\n';
    construct_model(coords, model, vars);

    cerr << "Optimizing..." << '\n';
    jointly_optimize(model, vars, response_n, response_d, response_n_d);

    cerr << "Generating output..." << '\n';
    parse_solution(v_min, v_max, coords, labels, vars, response_n, "n");
    parse_solution(v_min, v_max, coords, labels, vars, response_d, "d");
    parse_solution(v_min, v_max, coords, labels, vars, response_n_d, "n_d");
